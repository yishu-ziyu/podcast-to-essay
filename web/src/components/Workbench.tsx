  ...
  return (
    <section className={`workbench${episode ? ' has-episode' : ''}`}>
      ...
      {episode && <>
        ...
        {isWorking && <div className="focus-card processing"><div className="progress-head"><div><p className="card-eyebrow">{isPaused ? '已暂停' : '正在转录'}</p><h2>{isPaused ? `已暂停 · ${progressText(episode)}` : progressText(episode)}</h2></div><span className="pulse" /></div><div className="progress-track"><i style={{ width: episode.chunkCount ? `${Math.min(100, episode.completedChunks / episode.chunkCount * 100)}%` : '8%' }} /></div><p>{isPaused ? '已完成部分已保存，继续时从断点接续。' : '可离开页面，任务在后台保留。'}</p><button type="button" className="button quiet" disabled={controlling} onClick={toggleTranscription}>{controlling ? '请稍候' : isPaused ? '继续转录' : '暂停'}</button></div>}
