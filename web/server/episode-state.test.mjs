...
async function storeWith(files) {
  ...
  return { root, store: createEpisodeStore({ root, raw: path.join(root, 'raw'), cleaned: path.join(root, 'cleaned') }) };
}
...
test('失败后重试成功：不再显示转录失败', async () => {
  const { root, store } = await storeWith({ 'source.mp3': 'x', 'asr_raw.txt': '[00:00:00,000] Speaker 0: 文本\n', 'transcription-state.json': '{"state":"failed"}' });
...
test('最近一次转录失败且没有初稿：显示转录失败', async () => {
  const { root, store } = await storeWith({ 'source.mp3': 'x' });
